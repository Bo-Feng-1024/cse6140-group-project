...
def repair_randomized(graph, cover, timer=None):
    new_cover = set(cover)
    uncovered = [
    ...
    while uncovered:
        ...
        if timer and timer.is_time_up():
            # FAST FINISH: Just add one vertex from every remaining uncovered edge
            # This is O(E) and guaranteed to result in a valid cover
            for u, v in uncovered:
                if u not in new_cover and v not in new_cover:
                    new_cover.add(u)
            return new_cover
...
def local_search_2(graph, cutoff, seed):
    ...
    for _ in range(pop_size - 1):
        ...
        random_cover = repair_randomized(graph, set(), timer)
    ...
    while not timer.is_time_up():
        ...
        while len(new_population) < pop_size:
            ...
            child = repair_randomized(graph, child, timer)
