...
\begin{abstract}
The Minimum Vertex Cover (MVC) problem is a classic NP-complete
...
variant). The LP kernelization closes the previously catastrophic
gap on the dense bipartite instances on which the matching-based
...
budget required by the autograder. We report comprehensive
solution-quality and runtime results, qualified runtime
distributions (QRTDs), solution quality distributions (SQDs), and
box plots for the two local search variants on \texttt{large1} and
\texttt{large12}.
\end{abstract}
...
A strictly tighter root-level bound is given by the LP relaxation of
MVC. We solve this LP exactly in polynomial time via the classical
Nemhauser--Trotter
...
\texttt{large6} where the LP is fully integral, this preprocessing
alone returns the optimum without any branching.
...
\begin{figure}[t]
  ...
  \caption{BnB anytime convergence on four representative instances.
    ...
    closed by the LP kernel preprocessing alone (single trace point);
